...
export default function Module() {
  ...
  useEffect(() => {
    async function getPdfs() {
      try {
        const data = await client.fetch<PdfType[]>(pdfQuery);

        console.log("SANITY PDF:", data);

        setPdfs(data);
      } catch (error) {
        console.error("Error mengambil PDF:", error);
      } finally {
        setPdfLoading(false);
      }
    }
  ...
  useEffect(() => {
    async function getVideos() {
      try {
        const data = await client.fetch<VideoType[]>(videoQuery);

        console.log("SANITY VIDEO:", data);
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mt-14">
          <div className="mb-6">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Video tutorial dan materi pembelajaran untuk membantu proses belajar.
            </p>
          </div>
